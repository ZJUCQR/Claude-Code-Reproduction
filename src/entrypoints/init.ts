...
function isMissingOptionalTelemetryDependency(error: unknown): boolean {
  return (
    error instanceof Error &&
    (error.message.includes('@opentelemetry/sdk-metrics') ||
      error.message.includes('telemetry/instrumentation'))
  )
}
  ...
  try {
    ...
    void import('../services/analytics/growthbook.js').then(gb => {
      void import('../services/analytics/firstPartyEventLogger.js')
        ...
        .catch(() => {
          // Ignore telemetry logger startup failures in source snapshot builds.
        })
    })
  ...
  if (isEligibleForRemoteManagedSettings()) {
    ...
    if (getIsNonInteractiveSession() && isBetaTracingEnabled()) {
      void doInitializeTelemetry().catch(error => {
        if (isMissingOptionalTelemetryDependency(error)) {
          logForDebugging(
            `[3P telemetry] Optional telemetry dependency missing, skipping init: ${errorMessage(error)}`,
          )
          return
        }
        logForDebugging(
